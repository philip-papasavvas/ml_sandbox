...
| --- | --- | --- | --- |
| [SMS spam classifier](notebook-projects/spam_classifier_nlp.ipynb) | Classify SMS messages as spam or ham ([dataset](https://archive.ics.uci.edu/ml/datasets/sms+spam+collection)) | Text cleaning, tokenisation, bag-of-words / vectorisation; logistic regression, decision tree, and a Keras neural network | All four models >95% test accuracy; **best 98.5%** with the neural network ([Kaggle submission](https://www.kaggle.com/philpap/logisticregression-decisiontree-neuralnetwork)) |
| [Credit card churn](notebook-projects/credit_card_churn.ipynb) | Predict which bank customers will churn (BankChurners dataset, ~16% churn) | EDA, dummy-encoding of nominal features, feature scaling, a scikit-learn `ColumnTransformer` + `Pipeline` | Lifts accuracy from the **84% majority-class baseline to ~91%** via feature engineering and scaling; uses the confusion matrix to handle class imbalance |
| [Breast cancer — PCA](notebook-projects/breast_cancer_pca.ipynb) | Dimensionality reduction on the scikit-learn breast cancer dataset | Standardisation, Principal Component Analysis | **2 components** separate malignant/benign and capture ~63% of variance; **15 of 30** retain ~99% |
| [Diabetes — linear regression](notebook-projects/diabetes_linear_regression.ipynb) | Predict disease progression on the diabetes dataset | EDA, supervised linear regression | Documents that a plain linear model **underfits (R²≈0.47)** — a useful negative result motivating a stronger model |
| [WHOOP sleep EDA](notebook-projects/whoop_sleep_eda.ipynb) | First ML project on personal WHOOP fitness-tracker data | Real-world data cleaning, EDA, correlation analysis | Cleans a raw wearable export and relates recovery to HRV, resting heart rate and sleep score; prepares a model-ready dataset |
| [HYROX race simulation](notebook-projects/hyrox_simulation.ipynb) | Estimate finish times for a [HYROX](https://hyrox.com/) fitness race | Monte Carlo simulation; each run leg and station modelled with normal/uniform distributions | Builds per-station time distributions from real race splits (run and ski-erg legs modelled) |
